...
strategy ichAgentWard(const vector<strategy> focal, const vector<strategy> opponent)
{
   if (focal.size() < 11) // always start with C
   {
      return playD;
   }
   ...
   {
      if (opponent.at(opponent.size() - i) == playD)
      {
         return playD;
      }
   }
